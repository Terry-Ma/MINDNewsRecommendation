...
class NNModel:
    ...
    def init_data(self):
        ...
        self.config['model']['vocab_size'] = self.data_loader.vocab_size()
    ...
    def train(self):
        ...
        while self.cur_train_steps <= total_steps:
            ...
            for batch in self.data_loader.train_batch_iter():
                ...
                check_train_pred_y = np.concatenate((check_train_pred_y, np.array(batch_pred_y[:, 1].to('cpu'))))
                # check & val step
                if self.cur_train_steps > 0 and self.cur_train_steps % self.config['train']['steps_per_check'] == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for batch_pred_y, batch_y in self.data_loader.val_inference():
                            ...
                            check_val_steps += 1
                            check_val_pred_y = np.concatenate((check_val_pred_y, np.array(batch_pred_y[:, 1].to('cpu'))))
